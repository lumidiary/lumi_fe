          ...
          <Card style={{ width: '100%' }}>
            <PostInfo>
              <InfoBox>
                <TopRow>
                  <LeftSide>
                    ...
                    <DateLocation>
                      <DateText>
                        <LuCalendar size={18} style={{ color: '#757575', marginRight: '8px' }} />
                        {diary.createdAt.slice(0, 10)} (월요일)
                      </DateText>
                      <Location>
                        <FaMapMarkerAlt size={20} style={{ marginRight: '4px' }} />
                ...
                <TagList>
                  {diary.photos.map((photo, i) => (
                    <img key={i} src={photo.url} alt={`photo-${i}`} />
                  ))}
                </TagList>
              </InfoBox>
              ...
              <ImageFrame
                ...
                onPrev={() =>
                  setCurrentImageIdx(
                    idx => (idx - 1 + diary.photos.length) % diary.photos.length
                  )
                }
                onNext={() =>
                  setCurrentImageIdx(
                    idx => (idx + 1) % diary.photos.length
                  )
                }
              />
            </PostInfo>
